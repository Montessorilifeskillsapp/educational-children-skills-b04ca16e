...
const SocialLinks: React.FC<SocialLinksProps> = ({ variant = 'dark', className = '' }) => {
  const base =
    variant === 'light'
      ...
      : 'bg-white text-slate-700 hover:bg-slate-50 border-slate-200 hover:text-purple-600';
    ...
    <nav aria-label="Social media links" className={`flex items-center justify-center gap-3 ${className}`}>
      {socials.map(({ name, handle, href, Icon }) => (
        <a
          ...
          title={`${name}: ${handle}`}
          className={`inline-flex items-center justify-center w-11 h-11 rounded-full border shadow-sm transition-all duration-200 hover:scale-110 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-purple-500 ${base}`}
        >
